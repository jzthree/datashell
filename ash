#!/usr/bin/env python3
"""ash - Interactive array file explorer.

Navigate HDF5, NPZ, and NPY files like a filesystem with tab completion,
smart array previews, slicing, and built-in visualization.

Usage:
    ash data.h5               Interactive shell (HDF5)
    ash archive.npz           Interactive shell (NPZ)
    ash array.npy             Interactive shell (NPY)
    ash data.h5 -l            List root
    ash data.h5 -c 'tree'     Run command and exit
"""
...
try:
    ...
    print("ash requires numpy: pip install numpy", file=sys.stderr)
    sys.exit(1)
...
# ── NPY/NPZ wrappers (h5py-like interface) ──────────────────

class _FakeId:
    """Mimics h5py Dataset.id for storage size queries."""
...
class NpyDataset:
    """Wraps a numpy array (or mmap) to look like h5py.Dataset."""
    _is_dataset = True
    _is_group = False
    ...
    def __init__(self, arr, name='/data'):
        ...
        self.shuffle = False
        self.maxshape = arr.shape
        self.fillvalue = None
        self.attrs = _FakeAttrs()
        self.id = _FakeId(arr.nbytes if hasattr(arr, 'nbytes') else
...
class NpyRoot:
    """Wraps a single .npy file as a root group with one dataset 'data'."""
    _is_dataset = False
    _is_group = True

    def __init__(self, arr, filename=''):
        self._arr = arr
        self._ds = NpyDataset(arr, '/data')
        self.name = '/'
        self.attrs = _FakeAttrs()
...
class NpzGroup:
    """Wraps an npz file to look like h5py.Group/File."""
    _is_dataset = False
    _is_group = True

    def __init__(self, npz, name='/', filename=''):
        self._npz = npz
        self.name = name
        self.attrs = _FakeAttrs()
        self._filename = filename
        self._names = sorted(npz.files)
    ...
    def __getitem__(self, key):
        ...
        if key in self._npz.files:
            return NpyDataset(self._npz[key], name='/' + key)
        raise KeyError(key)
    ...
    def __contains__(self, key):
        return key.lstrip('/') in self._npz.files
...
def _is_group(obj):
    """Check if obj is a group (h5py.Group or wrapper)."""
    if _HAS_H5PY and isinstance(obj, h5py.Group):
        return True
    return getattr(obj, '_is_group', False)


def _is_dataset(obj):
    """Check if obj is a dataset (h5py.Dataset or wrapper)."""
    if _HAS_H5PY and isinstance(obj, h5py.Dataset):
        return True
    return getattr(obj, '_is_dataset', False)
...
def _is_string(dtype):
    if np.issubdtype(dtype, np.bytes_) or np.issubdtype(dtype, np.str_):
        return True
    if _HAS_H5PY:
...
class AShell(cmd.Cmd):
    ...
    def __init__(self, filename, no_color=False):
        ...
        self.file = self._open_file(filename, self.format)
        self.cwd = '/'
        self._prev_cwd = '/'
        ...
        fmt_tag = f" {self.format}" if self.format != 'hdf5' else ""
        self.intro = (
            f"\n{C.BOLD}ash{C.RESET} - {C.CYAN}{self.basename}{C.RESET}"
            f"  ({fsize}, {n_groups[0]} groups, {n_datasets[0]} datasets{fmt_tag})\n"
    ...
    @staticmethod
    def _open_file(filename, fmt):
        """Open file in the appropriate format."""
        if fmt == 'npy':
            arr = np.load(filename, mmap_mode='r')
            return NpyRoot(arr, filename)
        elif fmt == 'npz':
            npz = np.load(filename, allow_pickle=False)
            return NpzGroup(npz, filename=filename)
        else:
            if not _HAS_H5PY:
                print("HDF5 files require h5py: pip install h5py", file=sys.stderr)
                sys.exit(1)
    ...
    def _update_prompt(self):
        self.prompt = f"{C.BOLD}ash{C.RESET}:{C.BLUE}{self.cwd}{C.RESET}> "
    ...
    def _complete_path(self, text, line, begidx, endidx):
        ...
        if obj is None or not _is_group(obj):
            return []
        ...
        for name in obj:
            if name.lower().startswith(prefix.lower()):
                child = obj[name]
                suffix = '/' if _is_group(child) else ''
    ...
    def do_ls(self, arg):
        ...
        groups = [(n, obj[n]) for n in items if _is_group(obj[n])]
        datasets = [(n, obj[n]) for n in items if _is_dataset(obj[n])]
        for n, o in groups:
...
{C.BOLD}Visualization{C.RESET}  {C.DIM}(performance-safe: only loads visible data via stride slicing){C.RESET}
...
{C.BOLD}Metadata{C.RESET}
  ...
  {C.GREEN}attrs{C.RESET} [name]              Attributes (default: current group)
    ...
    def default(self, line):
        ...
        if obj is not None:
            ...
            if sl is not None and sl != 'ERROR':
                obj = self._get(base)
                if obj is not None and _is_dataset(obj):
                    self.do_cat(name)
                    return
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='ash - Interactive array file explorer (HDF5, npy, npz)',
    ...
    parser.add_argument('file', help='File to open (HDF5, .npy, .npz)')
    parser.add_argument('--no-color', action='store_true', help='Disable colors')
    parser.add_argument('-c', '--command', help='Run command and exit')
    parser.add_argument('-l', '--ls', action='store_true', help='List root contents')
    parser.add_argument('-d', '--data', metavar='DATASET',
                        help='Print dataset and exit')
    ...
    shell = AShell(args.file, no_color=args.no_color)
